...
export default function HeroSection() {
  ...
  return (
    <section ref={ref} id="home" className="relative z-10 min-h-screen flex items-center justify-center pt-20 overflow-hidden border-b border-[var(--color-line)]">
      {/* Field Background */}
      <div className="absolute inset-0 z-0 opacity-0 pointer-events-none animate-[fadeIn_1s_ease-out_0.8s_forwards]">
        <video 
          autoPlay 
          loop 
          muted 
      ...
      <div className="absolute inset-0 z-0 opacity-0 pointer-events-none bg-[radial-gradient(circle_at_50%_50%,rgba(140,255,90,0.05)_0%,transparent_70%)] mix-blend-screen animate-[fadeIn_1.5s_ease-out_0.8s_forwards]"></div>

      {/* Four Floodlights Sequence */}
      <div className="absolute inset-0 z-[65] pointer-events-none">
        {/* Light 1 (Top Left) */}
        <div className="absolute top-[-10%] left-[10%] w-[120px] h-[150%] origin-top rotate-[25deg] blur-[20px] opacity-0 animate-[lightFlicker_0.5s_ease-out_0.1s_forwards] mix-blend-screen"
             style={{ background: "linear-gradient(180deg, rgba(140,255,90,.6), transparent 60%)" }}></div>
        {/* Light 2 (Top Right) */}
        <div className="absolute top-[-10%] right-[10%] w-[120px] h-[150%] origin-top rotate-[-25deg] blur-[20px] opacity-0 animate-[lightFlicker_0.5s_ease-out_0.3s_forwards] mix-blend-screen"
             style={{ background: "linear-gradient(180deg, rgba(140,255,90,.6), transparent 60%)" }}></div>
        {/* Light 3 (Far Left) */}
        <div className="absolute top-[10%] left-[-5%] w-[120px] h-[150%] origin-top rotate-[45deg] blur-[15px] opacity-0 animate-[lightFlicker_0.5s_ease-out_0.5s_forwards] mix-blend-screen"
             style={{ background: "linear-gradient(180deg, rgba(140,255,90,.4), transparent 50%)" }}></div>
        {/* Light 4 (Far Right) */}
        <div className="absolute top-[10%] right-[-5%] w-[120px] h-[150%] origin-top rotate-[-45deg] blur-[15px] opacity-0 animate-[lightFlicker_0.5s_ease-out_0.7s_forwards] mix-blend-screen"
             style={{ background: "linear-gradient(180deg, rgba(140,255,90,.4), transparent 50%)" }}></div>
      </div>

      <motion.div 
        className="max-w-[1120px] mx-auto px-7 relative z-70 w-full flex flex-col items-center opacity-0 animate-[fadeInUp_1s_ease-out_0.8s_forwards]"
        style={{ y: textY }}
      >
        
        <div className="mb-6 flex items-center justify-center opacity-0 animate-[fadeIn_1s_ease-out_0.8s_forwards]">
        ...
        <p className="mt-[24px] text-[var(--color-muted)] text-[14px] md:text-[16px] leading-[1.6] text-center font-space tracking-[.05em] max-w-[600px] opacity-0 animate-[fadeIn_1s_ease-out_1s_forwards]">
          Gobichettipalayam&apos;s Premium Cricket, Football & Pickleball Arena
        </p>

        <div className="w-full grid grid-cols-2 sm:grid-cols-4 gap-[14px] mt-[80px] opacity-0 animate-[fadeInUp_1s_ease-out_1.2s_forwards]">
